...
def train(args, loader, encoder, generator, discriminator, g_ema, e_optim, g_optim, d_optim, device, save_dir):
    ...
    for idx in pbar:
        ...
        recon_pred = discriminator(recon_img)
        g_loss = g_nonsaturating_loss(recon_pred)# * args.adv
        loss_dict["g"] = g_loss
        ...
        if SummaryWriter :
            ...
            logger.add_scalar('G_loss/g', g_loss_val, i)
            logger.add_scalar('G_loss/path_length', path_length_val, i)
            logger.add_scalar('D_loss/d', d_loss_val, i)
        ...
        if get_rank() == 0:
            pbar.set_description(
                (
                    ...
                    f"e:{e_loss_val:.4f}; {l2_loss_val:.4f}; "
            ...
            if i % 100 == 0:
                with torch.no_grad():
                    ...
                    utils.save_image(
                        sample,
                        save_dir+f"sample_encoder/{str(i).zfill(6)}.png",
                        nrow=int(args.batch),
                        normalize=True,
                        range=(0, 1),
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', type=str, default='/ocean/projects/asc170022p/singla/MIMICCX-Chest-Explainer/stylegan2-pytorch/Experiment_MIMIC_CXR/Run_encoder_generator_multiGPU/', help='')
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default='/ocean/projects/asc170022p/singla/MIMICCX-Chest-Explainer/stylegan2-pytorch/Experiment_MIMIC_CXR/Run_encoder_generator_multiGPU/checkpoint/encoder_001000.pt',
    ...
    train(args, loader, encoder, generator, discriminator, g_ema, e_optim, g_optim, d_optim, device, save_dir = args.save_dir)
